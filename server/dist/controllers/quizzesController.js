...
exports.updateQuiz = (0, catchAsync_1.catchAsync)(async (req, res, next) => {
    ...
    // Only allow teachers to create quizzes
    if (((_a = req.user) === null || _a === void 0 ? void 0 : _a.role) !== "teacher") {
        return next(new appError_1.AppError("Only teachers can update quizzes", 403));
    }
    const { title, course, topic, dueDate } = req.body;
    ...
    const updateQuiz = await QuizModel_1.default.findByIdAndUpdate(quizId, { title, course, topic, dueDate }, { new: true, runValidators: true }).populate("creator", "name");
    res.status(201).json({
        status: "success",
        data: {
            quiz: updateQuiz,
...
exports.deleteQuiz = (0, catchAsync_1.catchAsync)(async (req, res, next) => {
    ...
    // Only allow teachers to create quizzes
    if (((_a = req.user) === null || _a === void 0 ? void 0 : _a.role) !== "teacher") {
        return next(new appError_1.AppError("Only teachers can delete quizzes", 403));
    }
    const quizId = req.params.quizId;
    const existingQuiz = await QuizModel_1.default.findById(quizId);
    if (!existingQuiz) {
    ...
    const deletequiz = await QuizModel_1.default.findByIdAndDelete(quizId);
    res.status(201).json({
        status: "success",
        data: null,
    });
